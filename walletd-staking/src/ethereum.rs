...
impl Staking for EthereumStaking {
    ...
    async fn get_validators(&self, limit: usize) -> StakingResult<Vec<ValidatorInfo>> {
        Ok((0..limit.min(10)).map(|i| ValidatorInfo { address: format!("0x{:040x}", i), name: format!("Lido Validator {}", i), commission: 10.0, total_stake: 32_000_000_000_000_000_000, delegators: 1000, uptime: 99.9, apy: 4.5, is_active: true }).collect())
    }
    async fn stake(&self, amount: u64, _validator: &str) -> StakingResult<Vec<u8>> { if amount < 32_000_000_000_000_000_000 { return Err(StakingError::MinimumStake(32_000_000_000_000_000_000)); } Ok(vec![0; 32]) }
