...
mod tests {
    ...
    // ============================================================================
    // SubaddressIndex Tests
    // ============================================================================

    #[test]
    fn test_subaddress_index_new() {
        let index = SubaddressIndex::new(1, 2);
        assert_eq!(index.major(), 1);
        assert_eq!(index.minor(), 2);
    }

    #[test]
    fn test_subaddress_index_default() {
        let index = SubaddressIndex::default();
        assert_eq!(index.major(), 0);
        assert_eq!(index.minor(), 0);
        assert!(index.is_zero());
    }

    #[test]
    fn test_subaddress_index_as_tuple() {
        let index = SubaddressIndex::new(3, 5);
        assert_eq!(index.as_tuple(), (3, 5));
    }

    #[test]
    fn test_subaddress_index_is_zero() {
        let zero = SubaddressIndex::new(0, 0);
        let non_zero = SubaddressIndex::new(0, 1);
    ...
    #[test]
    fn test_subaddress_index_display() {
        let index = SubaddressIndex::new(1, 2);
        let display = format!("{}", index);
        assert!(display.contains("Major: 1"));
        assert!(display.contains("Minor: 2"));
    }

    #[test]
    fn test_subaddress_index_equality() {
        let a = SubaddressIndex::new(1, 2);
        let b = SubaddressIndex::new(1, 2);
        let c = SubaddressIndex::new(2, 1);
    ...
    #[test]
    fn test_subaddress_index_clone() {
        let original = SubaddressIndex::new(5, 10);
    ...
    #[test]
    fn test_address_type_display_standard() {
        let display = format!("{}", AddressType::Standard);
        assert_eq!(display, "Standard Address");
    }

    #[test]
    fn test_address_type_display_subaddress_with_index() {
        let index = SubaddressIndex::new(1, 2);
        let display = format!("{}", AddressType::Subaddress(Some(index)));
        assert!(display.contains("Subaddress"));
        assert!(display.contains("Major: 1"));
    }

    #[test]
    fn test_address_type_display_subaddress_no_index() {
        let display = format!("{}", AddressType::Subaddress(None));
        assert_eq!(display, "Subaddress");
    }
    ...
    #[test]
    fn test_network_to_u8_mainnet() {
        assert_eq!(network_to_u8(&Network::Mainnet, &AddressType::Standard), 18);
        assert_eq!(network_to_u8(&Network::Mainnet, &AddressType::Subaddress(None)), 42);
    }

    #[test]
    fn test_network_to_u8_testnet() {
        assert_eq!(network_to_u8(&Network::Testnet, &AddressType::Standard), 53);
        assert_eq!(network_to_u8(&Network::Testnet, &AddressType::Subaddress(None)), 63);
    }

    #[test]
    fn test_network_to_u8_stagenet() {
        assert_eq!(network_to_u8(&Network::Stagenet, &AddressType::Standard), 24);
        assert_eq!(network_to_u8(&Network::Stagenet, &AddressType::Subaddress(None)), 36);
    }

    #[test]
    fn test_network_from_u8_mainnet() {
        assert_eq!(network_from_u8(18).unwrap(), Network::Mainnet);
        assert_eq!(network_from_u8(42).unwrap(), Network::Mainnet);
    }

    #[test]
    fn test_network_from_u8_testnet() {
        assert_eq!(network_from_u8(53).unwrap(), Network::Testnet);
        assert_eq!(network_from_u8(63).unwrap(), Network::Testnet);
    }

    #[test]
    fn test_network_from_u8_invalid() {
        assert!(network_from_u8(0).is_err());
        assert!(network_from_u8(255).is_err());
    }
    ...
    #[test]
    fn test_error_missing_spend_key() {
        let err = Error::MissingPublicSpendKey;
        assert!(err.to_string().contains("spend key"));
    }

    #[test]
    fn test_error_missing_view_key() {
        let err = Error::MissingPublicViewKey;
        assert!(err.to_string().contains("view key"));
    }

    #[test]
    fn test_error_network_parse() {
        let err = Error::NetworkParseError;
        assert!(err.to_string().contains("network"));
    }

    #[test]
    fn test_error_address_type_parse() {
        let err = Error::AddressTypeParseError;
    ...
    #[test]
    fn test_error_wrong_payment_id_style() {
        let err = Error::WrongPaymentIdStyle;
        assert!(err.to_string().contains("Payment ID"));
    }
    ...
    #[test]
    fn test_parse_mainnet_standard_address() {
        let address = Address::from_str(ADDRESS).unwrap();
        assert_eq!(address.network, Network::Mainnet);
        assert_eq!(address.format, AddressType::Standard);
    }

    #[test]
    fn test_parse_mainnet_subaddress() {
        let address = Address::from_str(SUBADDRESS_0_1).unwrap();
        assert_eq!(address.network, Network::Mainnet);
        assert!(matches!(address.format, AddressType::Subaddress(_)));
    }

    #[test]
    fn test_address_roundtrip() {
        let original = ADDRESS;
        let parsed = Address::from_str(original).unwrap();
        assert_eq!(parsed.to_string(), original);
    }

    // ============================================================================
    // Address Creation Error Tests
    // ============================================================================

    #[test]
    fn test_address_missing_spend_key() {
        let public_keys = MoneroPublicKeys {
            spend_key: None,
            view_key: Some(PublicKey::from_slice(&[1u8; 32]).unwrap()),
        };
        
        let result = Address::new(&Network::Mainnet, &public_keys, &AddressType::Standard);
        assert!(matches!(result, Err(Error::MissingPublicSpendKey)));
    }

    #[test]
    fn test_address_missing_view_key() {
        let public_keys = MoneroPublicKeys {
            spend_key: Some(PublicKey::from_slice(&[1u8; 32]).unwrap()),
            view_key: None,
        };
        
        let result = Address::new(&Network::Mainnet, &public_keys, &AddressType::Standard);
        assert!(matches!(result, Err(Error::MissingPublicViewKey)));
    }
}
