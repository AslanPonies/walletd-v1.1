...
mod tests {
    ...
    #[test]
    fn test_error_sending_zero_amount() {
        let err = Error::SendingZeroAmount;
    ...
    #[test]
    fn test_error_insufficient_funds_for_fee() {
        let err = Error::InsufficientFundsForFee {
            needed: 100,
            found: 50,
        };
        let msg = err.to_string();
    ...
    #[test]
    fn test_error_invalid_hard_fork() {
        let err = Error::InvalidHardForkVersionAssumption {
            found: 14,
            expected: 15,
        };
        let msg = err.to_string();
        assert!(msg.contains("hard fork"));
    }

    #[test]
    fn test_error_only_one_payment_id() {
        let err = Error::OnlyOnePaymentIdAllowed;
        assert!(err.to_string().contains("payment ID"));
    }

    #[test]
    fn test_error_key_image() {
        let err = Error::KeyImage;
        assert!(err.to_string().contains("Key image"));
    }

    // ============================================================================
    // Wallet Accessor Tests
    // ============================================================================
    ...
    #[test]
    fn test_address_type_display_subaddress() {
