...
/// Polygon RPC client for blockchain interactions
pub struct PolygonRpcClient {
    rpc_url: String,
    config: NetworkConfig,
}

/// Transaction receipt from Polygon
...
impl PolygonRpcClient {
    ...
    /// Get suggested gas price with Polygon-specific multiplier
...
mod tests {
    ...
    fn test_create_mainnet_client() {
        let client = PolygonRpcClient::mainnet("https://polygon-rpc.com");
        assert_eq!(client.config.chain_id, 137);
    }

    #[test]
    fn test_create_testnet_client() {
        let client = PolygonRpcClient::testnet("https://rpc-amoy.polygon.technology");
        assert_eq!(client.config.chain_id, 80002);
    }

    #[test]
    fn test_rpc_url_stored() {
        let url = "https://custom-rpc.example.com";
        let client = PolygonRpcClient::mainnet(url);
