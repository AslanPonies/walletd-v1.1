...
// ============================================================================
// SECURITY FIX v1.1.0 (H-01): ZeroizingSigningKey wrapper
// Ensures cryptographic keys are properly zeroized when dropped
// ============================================================================

/// A wrapper around SigningKey that ensures proper zeroization on drop.
/// 
/// SECURITY: Ed25519 SigningKey doesn't implement Zeroize directly,
/// so we wrap it to ensure the key bytes are securely cleared from memory.
pub struct ZeroizingSigningKey {
    inner: SigningKey,
}

impl ZeroizingSigningKey {
    /// Create a new ZeroizingSigningKey from raw bytes.
    ...
    /// Get the inner SigningKey for signing operations.
    ...
    /// Export key bytes (use with caution).
...
impl Drop for ZeroizingSigningKey {
    fn drop(&mut self) {
        // SECURITY: Properly zeroize the key bytes
        let mut key_bytes = self.inner.to_bytes();
        key_bytes.zeroize();
        // Note: The original inner key will also be dropped by Rust,
        // but we explicitly zero a copy to ensure the data is cleared
    }
}
